    ...
    for (global_ulong2 = 1u; global_ulong2 <= global_ulong1; global_ulong2++)
    {
        ...
        if (error_code != DPE_SUCCESS)
        {
            if (Action_code == DP_PROGRAM_ACTION_CODE)
            {
                ...
                #ifdef ENABLE_DISPLAY
                dp_display_text( "Failed to program device: Instruction timed out." );
                #endif
            }
            else if (Action_code == DP_VERIFY_ACTION_CODE)
            {
                ...
                #ifdef ENABLE_DISPLAY
                dp_display_text( "Failed to verify device: Instruction timed out." );
                #endif
            }
            else if (Action_code == DP_ERASE_ACTION_CODE)
            {
                ...
                #ifdef ENABLE_DISPLAY
                dp_display_text( "Failed to erase device: Instruction timed out." );
                #endif
            }
            else if (Action_code == DP_CHECK_BITSTREAM_ACTION_CODE)
            {
                ...
                #ifdef ENABLE_DISPLAY
                dp_display_text( "Failed to check bitstream: Instruction timed out." );
                #endif
            }
            #ifdef ENABLE_DISPLAY
            dp_display_text("\r\ncomponentNo: ");
        ...
        else
        {
            if ((global_ulong2 % 100u) == 0u)
            {
                ...
                if ((rtg4_poll_buf[0] &0x2u) != 0x2u)
                {
                    ...
                    #ifdef ENABLE_DISPLAY
                    dp_display_text( "Error, device is not ready");
                    dp_display_text("\r\nblockNo: ");
                    dp_display_value(global_ulong2, DEC);
                    dp_display_text("\r\nDATA_STATUS_RESULT: ");
                    dp_display_array(rtg4_poll_buf,4u,HEX);
