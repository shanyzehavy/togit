...
* 		purpose: This function is called at the begining to initialize starting and ending page addresses.
* Arguments: 
*		None. 
* Return value: 
* 		None.
...
void dp_get_page_data(DPULONG image_requested_address)
{
    ...
    */
    Serial2.print("get_page ");
    Serial2.print(image_requested_address);
    Serial2.print(" ");
    Serial2.println(return_bytes);
    // SZ This doesn't work:
    /*while (bytes_recieved < return_bytes) {
        while (Serial2.available() == 0) { 
        }
        page_global_buffer[bytes_recieved] = Serial2.read(); 
        bytes_recieved++;
    } */
    // Instead of the WHILE:
    int num_of_bytes_read_sz = Serial2.readBytes(page_global_buffer, return_bytes);
    if (num_of_bytes_read_sz < return_bytes) {
        dp_display_text("Read error");
        while (num_of_bytes_read_sz < return_bytes) {
            dp_display_text("Read error");
            num_of_bytes_read_sz = Serial2.readBytes(page_global_buffer, return_bytes);
        } 
    }
    
    start_page_address = image_requested_address;
    end_page_address = image_requested_address + return_bytes - 1u;
    dp_display_text("Read done");
