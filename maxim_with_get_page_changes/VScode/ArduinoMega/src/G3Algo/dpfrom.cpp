...
void dp_disturb_from(void)
{
    
    #ifdef ENABLE_DISPLAY
    dp_display_text("\r\nPerform FlashROM disturb...");
    #endif
...
void dp_verify_from(void)
{
    ...
    DataIndex=0U;
    global_uint1=0x80U; /* global_uchar2 could be used in place if FromAddressMaskIndex */
    ucFRomAddressMask = (DPUCHAR) dp_get_bytes(FRomAddressMask_ID,0U,1U);
    for (FromRowNumber=7; FromRowNumber >= 0 ;FromRowNumber--)
    {
        if (ucFRomAddressMask & global_uint1)
        {
            global_uchar2 = (DPUCHAR) FromRowNumber;
            opcode = ISC_UFROM_ADDR_SHIFT;
            IRSCAN_in();
            DRSCAN_in(0U,3U,&global_uchar2);
            goto_jtag_state(JTAG_UPDATE_DR,0u);
